...
const workflowSteps: {
  ...
  },
  {
    ...
    description: "Review full hierarchy.",
    detail:
      "Structure shows a manager version of the learning path, including draft, published, and archived items.",
    status: "Live",
    icon: ListChecks,
  },
];
...
const sortByOrder = <T extends OrderedItem>(items: T[]) =>
  [...items].sort((a, b) => {
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div className="mb-6 grid gap-3 md:grid-cols-4">
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-2 font-semibold text-slate-900">{programTag}</div>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        {sortByOrder(sections).map((section, sectionIndex) => {
          ...
          return (
            <div key={section.id} className="relative pl-16">
              <div className="absolute left-5 top-14 bottom-0 w-px bg-blue-100" />
              <div className="absolute left-0 top-0 flex h-11 w-11 items-center justify-center rounded-2xl bg-blue-600 text-sm font-bold text-white shadow-sm">
                {sectionIndex + 1}
              </div>
